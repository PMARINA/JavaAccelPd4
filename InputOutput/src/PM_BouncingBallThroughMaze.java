...
public class PM_BouncingBallThroughMaze { 
    public static void drawMaze(){
        double[] pts ={-1,0,-0.5,0,0.5,0.5,0.75,0.5,0.75,-0.5, 1, 0.75};
        for(int i = 0; i<pts.length-3; i+=2){
            StdDraw.line(pts[i],pts[i+1],pts[i+2],pts[i+3]);
        }
        /*
        for(int i = 0; i<pts.length -3; i++){
            StdDraw.line(pts[i],pts[i+1]-0.1,pts[i+2],pts[i+3]-0.1);
        }
        */
    }
